...
static lprofS_STACK_RECORD s_new_func;       /* used in 'enter_function' */
static lprofS_STACK_RECORD s_leave_ret;   /* used in 'leave_function' */
...
void lprofM_enter_function(lprofP_STATE* S, char *file_defined, char *fcn_name, long linedefined, long currentline) {
  ...
  lprofC_start_timer(&(s_new_func.time_marker_function_local_time));
  lprofC_start_timer(&(s_new_func.time_marker_function_total_time));
  s_new_func.file_defined = file_defined;
  if(fcn_name != NULL) {
    s_new_func.function_name = fcn_name;
    ...
    s_new_func.function_name = cur_name;
  } else {
    cur_name = (char*)malloc(sizeof(char)*(strlen(file_defined)+12));
    sprintf(cur_name, "%s:%li", file_defined, linedefined);
    s_new_func.function_name = cur_name;
  }	   
  s_new_func.line_defined = linedefined;
  s_new_func.current_line = currentline;
  s_new_func.local_time = 0.0;
  s_new_func.total_time = 0.0;
  lprofS_push(S, &(S->stack_top), s_new_func);
}
...
lprofS_STACK_RECORD *lprofM_leave_function(lprofP_STATE* S, int isto_resume) {
  ...
  s_leave_ret = lprofS_pop(S, &(S->stack_top));
  compute_local_time(&s_leave_ret);
  compute_total_time(&s_leave_ret);
