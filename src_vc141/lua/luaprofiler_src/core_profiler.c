...
static lprofS_STACK_RECORD *s_info;
static float s_function_call_time;
...
int lprofP_callhookOUT(lprofP_STATE* S) {
  ...
  s_info = lprofM_leave_function(S, 0);
  /* writing a log may take too long to be computed with the function's time ...*/
  lprofM_pause_total_time(S);
  s_info->local_time += s_function_call_time;
  s_info->total_time += s_function_call_time;
...
lprofP_STATE* lprofP_init_core_profiler(const char *_out_filename, int isto_printheader, float _function_call_time) {
  ...
  s_function_call_time = _function_call_time;
...
void lprofP_close_core_profiler(lprofP_STATE* S) {
  if(s_out_file) fclose(s_out_file);
  if(S) free(S);
}
...
void
luaprofiler_output(lprofP_STATE* S) {
	luaprofiler_logentry_t *entry;

	entry = S->log_list;
	while (NULL != entry) {

		entry->local_time += s_function_call_time;
		entry->total_time += s_function_call_time;

		char* source = entry->file_defined;
		if (source[0] != '@') {
			source = "(string)";
		}
		else {
