...
static int s_exit_id;
static int s_profstate_id;
...
static void callhook(lua_State *L, lua_Debug *ar) {
  ...
  lprofP_STATE* S;
  lua_pushlightuserdata(L, &s_profstate_id);
...
static void exit_profiler(lua_State *L) {
  ...
  lua_pushlightuserdata(L, &s_exit_id);
  lua_gettable(L, LUA_REGISTRYINDEX);
  lua_call(L, 0, 0);
}
...
static int luaprofiler_pause(lua_State *L) {
  lprofP_STATE* S;
  lua_pushlightuserdata(L, &s_profstate_id);
  lua_gettable(L, LUA_REGISTRYINDEX);
  S = (lprofP_STATE*)lua_touserdata(L, -1);
...
static int luaprofiler_resume(lua_State *L) {
  lprofP_STATE* S;
  lua_pushlightuserdata(L, &s_profstate_id);
  lua_gettable(L, LUA_REGISTRYINDEX);
  S = (lprofP_STATE*)lua_touserdata(L, -1);
...
static int luaprofiler_stop(lua_State *L) {
  lprofP_STATE* S;
...
static const luaL_Reg prof_funcs[] = {
  { "pause", luaprofiler_pause },
  { "resume", luaprofiler_resume },
  { "start", luaprofiler_init },
  { "stop", luaprofiler_stop },
  { NULL, NULL }
};
...
int luaopen_luaprofiler(lua_State *L) {
	...
	luaL_register(L, "luaprofiler", prof_funcs);
#else
