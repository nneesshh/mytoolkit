...
static inline int
luaRealIndex(lua_State* L, int index) {
...
luaL_getsubtable(lua_State *L, int idx, const char *fname) {
	...
	if (lua_istable(L, -1))
		...
		ridx = luaRealIndex(L, idx);
		lua_newtable(L);
		lua_pushvalue(L, -1);  /* copy to be left at top */
		lua_setfield(L, ridx, fname);  /* assign new table to field */
