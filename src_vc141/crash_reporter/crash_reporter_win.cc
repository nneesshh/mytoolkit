...
const MINIDUMP_TYPE kSmallDumpType = static_cast<MINIDUMP_TYPE>(MiniDumpWithFullMemory);
...
void CrashReporterWin::InitBreakpad(const std::string& daemon_executable_name, 
  ...
  if (wait_event != NULL) {
    DWORD dwRet = ::WaitForSingleObject(wait_event, 10);
    if (dwRet != WAIT_OBJECT_0) {
      // crash service not exist
      std::wstring daemon_cmdline = L"\"./";
	  daemon_cmdline.append(base::UTF8ToUTF16(daemon_executable_name));
	  daemon_cmdline.append(L"\" -application-name=");
	  daemon_cmdline.append(base::UTF8ToUTF16(product_name));
	  daemon_cmdline.append(L" -crashes-directory=");
	  daemon_cmdline.append(crashes_dir.AsUTF16Unsafe());
	  daemon_cmdline.append(L" -restart-cmdline=");

	  std::wstring sRestartCmdline = restart_cmdline_ + L" -crash_reboot=1";
	  base::ReplaceChars(sRestartCmdline, L"\"", L"\'", &sRestartCmdline);
	  sRestartCmdline = L"\"" + sRestartCmdline + L"\"";
	  daemon_cmdline.append(sRestartCmdline);

      // launch daemon
      base::LaunchOptions launch_options;
      launch_options.wait = false;
      launch_options.start_hidden = true;
      base::LaunchProcess(daemon_cmdline, launch_options);

	  // Wait until the crash service is started.
	  dwRet = ::WaitForSingleObject(wait_event, 10000);
    }
