...
bool CreateTopWindow(HINSTANCE instance,
  ...
  ::UpdateWindow(window);
  std::cerr << "window handle is " << window;
  g_top_window = window;
  return true;
}
...
bool CrashService::Initialize(const base::string16& application_name,
                              ...
                              const base::string16& restart_cmdline) {
  ...
  if (!dumper_) {
    std::cerr << "could not create dumper";
  ...
  //
  restart_cmdline_ = restart_cmdline;

  // Log basic information.
  std::cerr << "pipe name is " << pipe_name
          << "\ndumps at " << dumps_path_to_use.value();

  if (sender_) {
    std::cerr << "checkpoint is " << checkpoint_path.value()
  ...
  if (!dumper_->Start()) {
    std::cerr << "could not start dumper";
    if (security_attributes.lpSecurityDescriptor)
      LocalFree(security_attributes.lpSecurityDescriptor);
	abort();
    return false;
...
void CrashService::OnClientConnected(void* context,
  ...
  ProcessingLock lock;
  std::cerr << "client start. pid = " << client_info->pid();
...
void CrashService::OnClientExited(void* context,
  ...
  std::cerr << "client end. pid = " << client_info->pid();
  CrashService* self = static_cast<CrashService*>(context);
  ::InterlockedIncrement(&self->clients_terminated_);

  if (self->dump_raised_) {
	  // clear flag
	  self->dump_raised_ = false;

	  // restart
	  if (self->restart_cmdline_.length() > 0) {
		  // wait 5 seconds
		  ::Sleep(5000);
		  ...
		  launch_options.start_hidden = false;
		  base::LaunchProcess(self->restart_cmdline_, launch_options);
	  }
  }
...
void CrashService::OnClientDumpRequest(void* context,
  ...
  if (!file_path) {
    std::cerr << "dump with no file path";
	abort();
    return;
  }
  if (!client_info) {
    std::cerr << "dump with no client info";
	abort();
    return;
  }
  ...
  CrashService* self = static_cast<CrashService*>(context);
  if (!self) {
    std::cerr << "dump with no context";
	abort();
    return;
  }

  self->dump_raised_ = true;
  ...
  std::cerr << "dump for pid = " << pid << " is " << dump_location.value();

  if (!WriteCustomInfoToFile(dump_location.value(), map)) {
    std::cerr << "could not write custom info file";
	abort();
  }
...
DWORD CrashService::AsyncSendDump(void* context) {
  ...
  do {
    ::Sleep(kSleepSchedule[retry_round]);
    {
      ...
      std::cerr << "trying to send report for pid = " << info->pid;
    ...
    std::cerr << "dump for pid =" << info->pid << " crash2 id =" << report_id;
    --retry_round;
  } while (retry_round >= 0);

  if (!::DeleteFileW(info->dump_path.c_str()))
    std::cerr << "WARNING: could not delete " << info->dump_path;
...
int CrashService::ProcessingLoop() {
  ...
  std::cerr << "session ending..";
  while (ProcessingLock::IsWorking()) {
    ::Sleep(50);
  }

  std::cerr << "clients connected :" << clients_connected_
